import { Component } from "react";
import { Link } from "react-router-dom"

class NavBar extends Component {
    render(){
        return(
        <div className="NavBar">
            <Link to="/home">
                <button className="HomeButton">
                    <img src="data:image/png;base64," className="imageHome" alt=""/>
                </button>
            </Link>
            <Link to='/systemsettings'>
                <button className='Botonsettings'>
                    <img src='https://static.vecteezy.com/system/resources/thumbnails/006/542/150/small/setting-icon-design-tools-cog-gear-line-style-for-graphic-design-logos-web-sites-social-media-ui-mobile-app-free-vector.jpg' className='imageSettings' alt=""/>
                </button>
            </Link>
            <Link to='/profile'>
                <button className="BotonProfile">
                    <img src="https://static.vecteezy.com/system/resources/previews/005/544/718/original/profile-icon-design-free-vector.jpg" className="imageProfile" alt=""/>
                </button>
            </Link>
        </div>
        )
    }
}

export default NavBar;